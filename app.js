(function(){
  ...
  // Control buttons
  btnStart.addEventListener('click', ()=> updateRoom({ mode:'active', currentIndex:0, accept:true }));
  btnStop.addEventListener('click', ()=> updateRoom({ mode:'ended', accept:false }));
  ...
  async function ensureRoomExists(id){
    ...
    if(!snap.exists){
      await ref.set({ title:'새 세션', mode:'idle', currentIndex:-1, accept:false, questions:[] });
    }
  }
  ...
  function buildQuestionRow(no, q){
    ...
    wrap.innerHTML = `
      ...
      <div class="mcq ${q?.type==='short'?'hidden':''}">
        ...
        <div class="row wrap">
          <label>정답 번호</label>
          <input class="ansIndex" data-no="${no}" type="number" min="1" max="10" value="${(q?.answerIndex ?? 1)+1}" style="width:100px" />
        </div>
      </div>
